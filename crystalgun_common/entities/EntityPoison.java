...
public class EntityPoison extends Entity implements IProjectile
{
	...
	private ArrayList effectsextra;
    ...
    public EntityPoison(World par1World)
    {
        super(par1World);
    ...
    public EntityPoison(World par1World, EntityLiving par2EntityLiving, ArrayList effect, ArrayList effectextra, int redColor, int greenColor, int blueColor)
    {
        super(par1World);
        effects = effect;
        effectsextra = effectextra;
        red = redColor;
        green = greenColor;
        blue = blueColor;
    ...
    public EntityPoison(World par1World, double par2, double par4, double par6)
    {
        super(par1World);
    ...
    public void onUpdate()
    {
        ...
        {
        	CrystalGunParticleHandler.spawnParticle("Poison", this.posX, this.posY, this.posZ, this.motionX - 0.4546F, this.motionY - 0.4546F, this.motionZ - 0.4546F, red, green, blue);
        }
    ...
    protected void onImpact(MovingObjectPosition par1MovingObjectPosition)
    {
    	...
    	{
    		if (par1MovingObjectPosition.entityHit instanceof EntityLiving && this.getThrower() instanceof EntityLiving)
			{
    			for(int i = 0; i < effects.size(); i++)
    			{
    				if(effects.get(i) == "Fire")
    				{
    					ArrayList fireheal = (ArrayList) effectsextra.get(i);
    					damage = (Integer) fireheal.get(0);
    					par1MovingObjectPosition.entityHit.setFire((Integer) fireheal.get(1));
    				}
    				if(effects.get(i) == "Heal")
    				{
    					ArrayList fireheal = (ArrayList) effectsextra.get(i);
    					EntityLiving hit = (EntityLiving)par1MovingObjectPosition.entityHit;
    					EntityLiving shooting = (EntityLiving)this.getThrower();
    					damage = (Integer) fireheal.get(0);
    					int heal = (Integer) fireheal.get(1);
    						
    					if(0 <= hit.getHealth() && hit.getHealth() < damage)
    					{
    						shooting.heal(hit.getHealth());
    					}
    					else if(hit.getHealth() < 0)
    					{
    				...
    				if(effects.get(i) == "Potion")
    				{
    					ArrayList potion = (ArrayList) effectsextra.get(i);
    					EntityLiving hit = (EntityLiving)par1MovingObjectPosition.entityHit;
    					damage = (Integer) potion.get(0);
	    					
                        hit.addPotionEffect(new PotionEffect((Integer) potion.get(1), (Integer) potion.get(2), (Integer) potion.get(3)));
    				}
    				if(effects.get(i) == "Knockback")
    				{
    					ArrayList fireheal = (ArrayList) effectsextra.get(i);
    					EntityLiving hit = (EntityLiving)par1MovingObjectPosition.entityHit;
    					damage = (Integer) fireheal.get(0);
    					int knockback = (Integer) fireheal.get(1);
    					float knock = MathHelper.sqrt_double(this.motionX * this.motionX + this.motionZ * this.motionZ);
    					
    					if (knock > 0.0F)
    					{
    						hit.addVelocity(this.motionX * (double)knockback * 0.6000000238418579D / (double)knock, Math.abs(this.motionY * 0.04267D), this.motionZ * (double)knockback * 0.6000000238418579D / (double)knock);
    					}
    				}
    			}
    		}
    	...
    	for(int k = 0; k < 5; k++)
    	{
    		...
    		CrystalGunParticleHandler.spawnParticle("Poison", this.posX, this.posY, this.posZ, 0, 0, 0, red, green, blue);
    		this.worldObj.spawnParticle("smoke", this.posX, this.posY, this.posZ, 0, 0, 0);
    	}
    }
